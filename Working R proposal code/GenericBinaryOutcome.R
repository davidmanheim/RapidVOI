...
###############################
# takes P_t_U (matrix) as an input 
...
#
# P_t_U (matrix) - number of rows = number of sims , number of columns = number of treatmetns
# Benefit <- TRUE    #  
#
...
# inputs for booklet proposal - approximated CHECK!!!!
...
# MCD inputs
MCD_t <- c(0) # remember MCD is a relative input - if two treatmetns => just one MCD
...
# mock data
#P_t_U <- matrix(c(rep(0.3, 10), rnorm(10, 0.3, 0.05), rnorm(10, 0.3, 0.05)), ncol = 3)
#MCD_t <- c(0.3, 0.4)
#Benefit = TRUE
...
UtilMatrixMCD.fn <- function(P_t_U, MCD_cutoff, Utilisation_t){
  
  # P_t_U = the "true" result on the probability scale for base and new treatment **NB needs to be a matrix!!
  ...
  for (i in 1:Nrows){
    # for each row assign a 1 to the new treatment which is above the cutoff and is the best of the new treatments
    UtilMatrix[i,] <- sapply(P_t_U[i,], function(y) ifelse(y>=MCD_cutoff[i] && y==max(P_t_U[i,]), 1, 0))
    
    if(max(P_t_U[i,])<MCD_cutoff[i]){ # if nothing is above the cutoff then just stick with current utilisation
      UtilMatrix[i,] <- Utilisation_t
    }
  }
...
#utilisation_matrix <- UtilMatrix.fn(P_t_U, rep(0.32, MCouter), c(1,0))
...
# test EVTPIadjImp.fn
#EVTPIadjImp.fn(utilisation_matrix, P_t_U)
# unadjusted EVTPI
#mean(apply(P_t_U, 1, max))
...
GenericBinaryOutcome.v1 <- function(P_t_U ,Benefit,MCD_t , Incidence, 
  ...
  EVTPIadjImp <- EVTPIadjImp.fn(UtilMatrix, NB_t_U)
...
# output P_t_U matrix (input to the above model)
